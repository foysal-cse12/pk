...
class Factory_model extends CI_Model {
	...
	public function office_new_order_data($data)
	{
		...
		redirect('factory/factory_order','refresh');
	...
	public function order_report_delete($id,$employee_id)
	{
		...
		redirect('factory/product_report/'.$employee_id,'refresh');
	}
	...
	public function order_data_update($data)
	{
		...
		$this->db->update('order_list',$update);
		 redirect('factory/product_report/'.$client_id,'refresh');
	}
	...
	public function  particulars_data($data)
	{
		...
		redirect('factory/particular_list','refresh');
	...
	public function check_inventory($data)
	{
		$check = array('product_category'=>$data['product_category']);
		$this->db->select('*');
		$this->db->where($check);
	...
	public function insert_inventory_data($data)
	{
     ...
     // $data= array('product_name'=>$product_name,'product_category'=>$product_category,'suppliar'=>$suppliar,'quantity'=>$quantity,'unit_price'=>$unit_price);
      $data= array('product_category'=>$product_category,'suppliar'=>$suppliar,'quantity'=>$quantity,'unit_price'=>$unit_price);
      $this->db->insert('inventory',$data);	
      
	  //redirect('home/index','refresh');
	  redirect('factory/inventory_list','refresh');
	}

	public function inventory_list_pagination()
	{
		$this->db->select('*');		
		$result = $this->db->get('inventory');
		return $result->num_rows();
	...
	public function inventory_list($limit, $start)
	{
		$this->db->select('*');
		$this->db->limit($limit, $start);
		$result = $this->db->get('inventory');
	...
	public function edit_inventory_data($data)
	{
	 // $product_name = $data['product_name'];
    //  $product_category = $data['product_category'];
      $suppliar = $data['suppliar'];
      ...
      $id = $data['id'];
      //$data= array('product_name'=>$product_name,'product_category'=>$product_category,'suppliar'=>$suppliar,'quantity'=>$quantity,'unit_price'=>$unit_price);
      $data= array('suppliar'=>$suppliar,'quantity'=>$quantity,'unit_price'=>$unit_price);
      $this->db->where('id',$id);
	...
	public function facrory_order_edit($id)
	{
	...
	public function product_quantity($product)
	{
		$this->db->select('quantity');
		$this->db->where('product_category',$product);
		$result = $this->db->get('inventory');
		return $result->row_array();
	...
	public function facrory_order_delete($id)
	{
		...
		$this->db->where($rule);
		$this->db->delete('order_list'); 
		redirect('factory/factory_view_order','refresh');
	}
